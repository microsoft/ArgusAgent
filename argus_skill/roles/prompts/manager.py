...
def build_front_door_prompt(text: str, *, active_mission: bool = False) -> str:
    ...
    return (
        ...
        "controls, bounded read-only inspection, and—only with no active mission—one "
        "finite local workspace microtask with one explicit deliverable and local "
        "acceptance check. TEAM for substantive or multi-source research (including "
        "company due diligence), multiple artifacts, ambiguity, network/install/git/"
        "publish work, experiments, background work, or anything needing review. "
        "When unsure, choose TEAM.\n\n"
        "SELF_MODE: SELF uses REPLY when no tools are needed, EXECUTE only for that "
        "local microtask, otherwise INSPECT. TEAM uses NONE. REPLY is the full "
        "user-facing answer, only for SELF/REPLY.\n\n"
