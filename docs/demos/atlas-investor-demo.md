...
在一个新的空工作区会话中，进入地图，复制下面的完整内容。保持“自动”模式即可；本次彩排已通过自动分类进入软件任务。
...
1. 消息从输入框飞向任务地图。Planner 产生真实任务节点和依赖；任务分解不再只是一段计划文字。
...
完整生成包含代码实现、实验、浏览器检查和独立审查，需要等待模型工作；不要将成品的交互速度描述为从零生成速度。第一轮完整运行约 15 分钟，多任务彩排耗时应以实际记录为准。没有使用预制地图动画伪装任务执行，示例城市数据明确标注为合成数据。

## 验证范围

已检查桌面与手机尺寸下的 Chromium 渲染与交互，以及 Atlas 在 WebKit 中的任务地图、Markdown、文件链接、图生长和减少动态效果设置。测试覆盖本次演示的实际用户流程，不表示每一种任意生成应用都无需后端运行环境。
