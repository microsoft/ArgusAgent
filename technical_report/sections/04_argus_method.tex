...
We use \emph{evidence-driven} for the overall control policy that decides whether to
persist, stop, or pivot, and \emph{verification-gated} for the narrower admission
condition on reusable updates. The first term is chosen deliberately over
goal-driven. A goal-driven controller treats the stated objective as the authority
and every departure from it as error to be suppressed; an evidence-driven controller
treats the objective as the current best hypothesis about what should be built, and
allows evidence to overturn it. On tasks whose objective was underdefined at the
start, the second is the correct posture, because the objective a human expert wrote
before the work began is itself a claim that can be wrong. We use \emph{runtime self-evolution} in a deliberately
