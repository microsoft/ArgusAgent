...
1. **Template, preamble, and anonymity**
   ...
   - Target the full 7 pages of TECHNICAL CONTENT excluding References and the Reproducibility Checklist; do not pad pilot evidence into a full-paper shell. References and the Reproducibility Checklist sit on ADDITIONAL pages that do NOT count toward the 7 and are uncapped: do not enforce any maximum page count for that end matter. The 7 pages are a **ceiling, not a quota**. If Conclusion moves after page 7 or `main_content_pages` exceeds 7.0, stop expanding and perform a page-map reflow: compress repeated body paragraphs, merge tiny sections, and shorten captions/tables while preserving reader-facing abstract, Introduction, Method, and Experimental Setup substance. A paper that ends earlier is not deficient for ending earlier — treat an early Conclusion as a prompt to ask whether any method, evidence, or argument a reader needs is actually missing, and expand only when something is. Never pad to reach a page number: filler, repeated caveats, and stretched prose are worse than a shorter complete paper. Text after References never repairs a genuinely thin body.
...
2. **Section order and completeness**
   - Conclusion must render by page 7; if it lands on page 8 or later, reduce or move earlier body material instead of adding more Introduction/Method prose. There is no lower bound — a complete paper that concludes earlier is fine, and padding to reach a page number is not. It must arrive through normal body flow, not a forced manual page break (which is forbidden anyway).
...
3. **Compile/PDF cleanliness and bibliography**
   ...
   - Final-ready drafts must cite every material premise, the nearest competitors, and the contradictions a reader would raise, with each entry verified. Bibliography depth is proportional to what the paper claims — there is no entry-count or References-page-count floor, and a complete short paper is not deficient for having a shorter reference list.
