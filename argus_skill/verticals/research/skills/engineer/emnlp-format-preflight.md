...
2. **Section order and completeness**
   - Conclusion must render by page 8; if it lands on page 9 or later, reduce or move earlier body material instead of adding more Introduction/Method prose. There is no lower bound — a complete paper that concludes earlier is fine, and padding to reach a page number is not. It must arrive through normal body flow, not a forced manual page break immediately before the section.
