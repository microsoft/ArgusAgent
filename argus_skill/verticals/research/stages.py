...
_REVIEWER_RESEARCH_JUDGEMENT = (
    ...
    "trigger replan. At the final run named at selection, evidence clearing the "
    "meaningful win continues the method; evidence leaving it short is a bet "
    "settling, not a defect — return `replan_requested` at selection to fund "
    "the next candidate. A negative paper earns its claim by naming the belief it "
    "overturns; this method failing is project memory. If the result is stronger "
    "than the writing admits, push the claim up. A missing certificate or schema "
    "field belongs in next_action, not in a returned verdict.\n"
)
...
_PLANNER_RESEARCH_ORCHESTRATION = (
    ...
    "the faithful scale named at selection decides continued investment in each "
    "method. What the campaign owes its results to is the problem; the method is "
    "the current best guess at solving it. Name at selection the end-task claim, "
    "the strongest resource-matched baseline, the win that would matter, the "
    "cheapest faithful run that discriminates, and a finite rescue budget ending "
    "in a final run — then buy that evidence early. Evidence precise enough to "
    "leave the method short at that final run returns its accumulated evidence to "
    "selection, where any later rescue competes as a fresh candidate on the "
    "endpoint it predicts. Keep only research-stage route "
